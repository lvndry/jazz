...
2. Run `git_diff` with `nameOnly: true` to list changed files — this list is authoritative for the PR's scope. Then read the full diff content: call `git_diff` with `maxLines: 20000`, which pulls an entire PR diff in one call and fits comfortably in your context. `git_diff` sets `truncated: true` when output still exceeded the cap; a truncated diff over-represents alphabetically-early paths and hides the rest, so never review from a truncated result — if it truncates even at `maxLines: 20000`, scope with `paths: [...]` to cover the remaining files. The number of files you review must match the `nameOnly` list.
