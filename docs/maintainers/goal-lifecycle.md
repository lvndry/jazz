...
Goals start in two ways: `/goal <objective>` asks for one directly, and in chat a deterministic imperative-phrase heuristic, [`mayBeGoalRequest`](../../packages/cli/src/chat/commands/goal.ts), offers the goal flow for turns that look like longer objectives. The offer defaults to no, declining runs the turn normally, and a declined offer is not repeated in that session, so a false positive costs one prompt and never replaces a normal turn. The heuristic excludes common questions and hypotheticals but is not a semantic classifier. The planner is a strict-schema model call: a malformed or failed plan is shown as a visible failure and never starts work, and a proposal with material open questions is shown as questions instead of a goal.
...
A claimed cycle lives on the record as `cycle`: its run ID, the process that started it, the message offset where it began, and any pause or cancel requested while it runs. The field exists exactly while the cycle's outcome and spend are not yet folded into the goal, and the record schema rejects states that contradict it (an `awaiting-input` or `stopping` goal without a cycle, a terminal goal with one). The [goal states](../../packages/core/src/agent/goal/goal-state.ts) are:
...
A run ending with a final answer means only that the run ended. The cycle prompt lists the accepted success criteria by number and asks for a strict disposition: `continue`, `complete`, `question`, or `blocked`. A `complete` disposition must cite every criterion by number with a quote that appears in tool output from the current cycle. Matching ignores whitespace differences and surrounding quote marks and allows `...` elisions whose fragments appear in order; a quote of fewer than eight characters is not evidence. Paraphrase does not match.

When the disposition is missing, malformed, or fails the evidence check, Jazz makes one schema-constrained repair call over the cycle's answer and a capped set of its tool outputs. The repaired disposition passes the same check, so repair can recover a misformatted answer but cannot invent a completion. If it still does not validate, the goal moves to `review-required` with the reason. A `question` disposition also stops for review; `/goal resume <id> <answer>` carries the answer into the next cycle's prompt.
...
Persist a cycle claim before starting the run and persist its transcript, evidence, usage, and next goal state before making another cycle eligible. Each claim uses a stable run ID and records its process owner. After a crash, a dead or unverifiable owner, missing run record, or unreconciled terminal run moves the goal to `review-required`; the prompt is never replayed automatically. This is especially important for shell and network actions that cannot be rolled back.
