...
| `/goal <objective>` | Draft a plan for a longer objective; once you accept it, the daemon works through it in cycles. `/goal list` shows progress and budget, `/goal pause <id>` and `/goal cancel <id>` stop it, `/goal resume <id> [note]` continues it (the note answers its question or steers the next cycle, and resuming a budget-limited goal extends its budget). |
