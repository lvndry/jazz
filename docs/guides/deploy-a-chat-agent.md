...
All four platforms give you the same thing: per-chat memory, per-chat `/model` and `/persona`,
reminders, and attachments. They differ in where they can run and what the app can show.
Telegram and Discord are Docker services. iMessage can use a hosted Photon line or your own
Apple account on a Mac. WhatsApp links to your account the way WhatsApp Web does.
...
## iMessage with a hosted line

The default gives the agent a line through [Photon](https://photon.codes), so it can run on
Linux or macOS and does not answer as your personal Apple account.

```bash
jazz imessage
```

Create a Photon project, then enter its project id and secret when Jazz asks. The first run also
asks which handles may contact the agent. For a noninteractive server, provide
`PHOTON_PROJECT_ID`, `PHOTON_PROJECT_SECRET`, and `PHOTON_ALLOWED_HANDLES` in the process
environment. Messages pass through Photon; use the local mode below when that is not acceptable.

Full setup and limitations: [`packages/photon-bot/README.md`](../../packages/photon-bot/README.md).

## iMessage through your own Mac

Local mode runs on a Mac running macOS 14 or newer, signed into iMessage, awake and logged in.
Unlike Photon, it uses **your** Apple account: the same line your friends already text.
...
`~/Library/LaunchAgents/com.github.lvndry.jazz.imessage.plist`, then run `jazz imessage --local`
...
can switch a conversation to any of the 22 providers Jazz supports. Anthropic, Gemini,
