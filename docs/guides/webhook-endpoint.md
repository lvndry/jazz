...
webhook different from a `jazz run` in a shell script — a fixed prompt the caller cannot
replace, a per-door bearer token, a tool ceiling for a caller who is not you, and an optional
...
| `agentId`        | Which agent wakes — its id or its name                                                   |
...
Leave `{{payload}}` out and the payload is appended at the end instead — the same quoting,
just less control over where it sits.
...
Copy the value now. If you lose it, run the command again to mint a new one — it overwrites
...
the next request — the list is read per call, so there is nothing to restart.
...
unknown, `costIncomplete: true` rides alongside it rather than being folded in — the figure is
...
first — a tunnel for a trial, a reverse proxy with TLS for anything lasting — and read
...
remembering the last deploy buys you nothing. When deliveries are turns in one conversation —
a support thread, a chat relay — make the door `threaded` and tell it which thread each fire
belongs to:
...
- **`disclosure`** is a ceiling on what an answer may *reveal*. `internal` (the default) is
  read-only tools that describe the shape of the machine — what exists, what is installed,
  what the web says — but not the contents of your files or your memory. `public` is less,
