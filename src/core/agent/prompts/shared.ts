...
- execute_command: Timeout defaults to 15 minutes. Dangerous commands (rm -rf, sudo, fork bombs, etc.) are blocked. Interpreter inline-code flags (python3 -c, node -e, ruby -e, etc.) are also blocked — write the script to a temp file (e.g. /tmp/check.py) and run that instead. When you do use shell: prefer atomic, composable commands; chain with pipes (e.g. cat file | grep pattern | head -n 5, or jq for JSON).
