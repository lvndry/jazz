...
- spawn_subagent: Use persona 'coder' for code search/editing/git tasks, 'researcher' for web search/information gathering, 'default' for general tasks. If a delegatable_agents roster appears in your prompt and one of its entries fits the task better than any persona, pass that agent's exact name as 'agent' instead. Match the model to the task's difficulty via 'model': cheap models for mechanical or well-scoped work, capable ones for subtle work — call list_models for prices and capabilities rather than guessing model names. Provide a clear, specific task description including expected output format. Use subagents liberally for investigation — mapping call sites, finding all affected files, understanding architecture — before you start editing.
- list_models: Lists models across configured providers with capabilities, context window and per-million-token prices, cheapest first. Filter with requires (e.g. ['tools']) and minContextWindow. Use it before overriding a sub-agent's model, not for idle curiosity.
