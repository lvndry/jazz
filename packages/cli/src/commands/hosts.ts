...
export function listHostsCommand() {
  return Effect.gen(function* () {
    ...
    for (const host of hosts) {
      process.stdout.write(`${host.name}\t${host.sshTarget}\t${host.workspacePath}\n`);
    }
  });
}

/** Register one checked SSH alias and existing remote workspace. */
export function addHostCommand(name: string, sshTarget: string, workspacePath: string) {
  return Effect.gen(function* () {
    const candidate: HostProfile = { name, sshTarget, workspacePath };
    const checked = checkConfigWrite("hosts", [candidate]);
    if (!checked.ok) throw new Error(`Invalid host registration: ${checked.problem}`);
    const config = yield* AgentConfigServiceTag;
    const current = (yield* config.appConfig).hosts ?? [];
    if (current.some((host) => host.name === name)) throw new Error(`Host ${name} already exists`);
    const next = [...current, candidate];
    const revision = yield* config.revision;
    yield* config.set("hosts", next);
    if ((yield* config.revision) === revision) throw new Error("Could not save host registration");
...
export function removeHostCommand(name: string) {
  return Effect.gen(function* () {
    const config = yield* AgentConfigServiceTag;
    const current = (yield* config.appConfig).hosts ?? [];
    if (!current.some((host) => host.name === name)) throw new Error(`No host named ${name}`);
    const revision = yield* config.revision;
    yield* config.set(
      "hosts",
...
export function doctorHostCommand(name: string) {
  return Effect.gen(function* () {
    ...
    if (host === undefined) throw new Error(`No host named ${name}`);
 ...
 */
export function importRemoteSecretCommand(secretPath: string) {
  return Effect.gen(function* () {
    ...
    if (backend === "none") throw new Error("Remote secret storage is disabled");
    const value = yield* Effect.tryPromise(async () => {
      ...
      for await (const chunk of process.stdin as AsyncIterable<Uint8Array>) {
        ...
        bytes += data.length;
        if (bytes > 16 * 1024) throw new Error("Secret exceeds the import limit");
        chunks.push(data);
      }
    ...
    if (!stored) throw new Error("Remote secret store refused the write");
