...
beforeAll(async () => {
  ...
  for (const ignored of ["node_modules", "dist", "build", ".git"]) {
...
describe("scanFilePickerEntries bounds", () => {
  ...
  test("skips dependency, output, and .git directories", async () => {
  ...
  test("includes dotfiles", async () => {
    ...
    expect(entries.map((entry) => path.basename(entry.path))).toEqual([".hidden.ts"]);
  });

  test("skips .env and .env.*", async () => {
    const entries = await scan({ query: "env" });
    expect(entries).toHaveLength(0);
  });
});
