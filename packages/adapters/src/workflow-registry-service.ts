/**
 * Implements `WorkflowRegistryService`: the workflow half of the marketplace.
 *
...
export class WorkflowRegistryServiceImpl implements WorkflowRegistryService {
  private readonly catalog: MarketplaceCatalog<RegistryWorkflowEntry>;

  constructor(options?: WorkflowRegistryServiceImplOptions) {
    this.catalog = new MarketplaceCatalog<RegistryWorkflowEntry>({
      kind: "workflow",
      ...
      function* (this: WorkflowRegistryServiceImpl) {
        ...
        if (definition === null) {
          return yield* Effect.fail(
            new ValidationError({
              field: "frontmatter",
              message: `Marketplace workflow "${entry.name}" is not a workflow Jazz can run: its frontmatter must declare name and description`,
              value: sourceUrl,
        ...
        if (prompt.length === 0) {
          return yield* Effect.fail(
            new ValidationError({
              field: "prompt",
              message: `Marketplace workflow "${entry.name}" has an empty prompt`,
              value: sourceUrl,
        ...
        if (prompt.length > MAX_PROMPT_LENGTH) {
          return yield* Effect.fail(
            new ValidationError({
              field: "prompt",
              message: `Marketplace workflow "${entry.name}" exceeds the ${MAX_PROMPT_LENGTH}-character prompt limit`,
