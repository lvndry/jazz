...
export async function getModelsDevProviderModels(
  ...
  if (!data) {
    throw new Error(
      isOfflineMode()
        ? "Model catalog unavailable: JAZZ_OFFLINE is set and no cached catalog exists. Local providers (ollama, llamacpp, vllm, sglang) list models without the catalog."
